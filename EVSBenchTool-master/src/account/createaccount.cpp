...
//    strftime( tmp, sizeof(tmp), "%Y/%m/%d %X %A 本年第%j天 %z",localtime(&t) );
    ...
    for (int i = 0; i < threadNum; ++i) {
        ...
        file.write("active:");
        file.write(active.get_eos_public_key().c_str());
        file.write(" ");
        file.write(active.get_wif_private_key().c_str());
        file.write("-------");
        file.write("owner:");
        file.write(owner.get_eos_public_key().c_str());
        file.write(" ");
        file.write(owner.get_wif_private_key().c_str());
        file.write("\n");
        qDebug() << "owner:" << owner.get_eos_public_key().c_str() << " " << owner.get_wif_private_key().c_str() << endl;
        qDebug() << "active" << active.get_eos_public_key().c_str() << " " << active.get_wif_private_key().c_str() << endl;
...
QByteArray CreateAccount::packGetRequiredKeysParam()
{
    ...
    EOSNewAccount newAccount(super_account.toStdString(), newAccountName.toStdString(),
                             keys.at(0).get_eos_public_key(), keys.at(1).get_eos_public_key(),
                             super_account.toStdString());
    ...
    signedTxn = ChainManager::createTransaction(EOS_SYSTEM_ACCOUNT, newAccount.getActionName(), std::string(hexData.begin(), hexData.end()),
                                                ChainManager::getActivePermission(super_account.toStdString()), getInfoData);
